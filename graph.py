...
class Graph:
    def __init__(self, nodes, edges, energy_price, parameters) -> None:
        ...
        self.vl_allocated = dict()
        self.node_power_consumption = 150
        self.parameters = parameters

        for a, b in edges:
            self.edge_weights[(a, b)] = random.randint(lower_edge, upper_edge)
            self.edge_weights[(b, a)] = self.edge_weights[(a, b)]
            self.delay[(a, b)] = random.randint(
                parameters.lower_delay, parameters.upper_delay
            )
        ...
        for path in all_paths:
            self.vl_allocated[tuple(path)] = False
    ...
    def printAllPaths(self, s, d, weight):
        ...
        self.printAllPathsUtil(
            s, d, visited, weight, path, all_path
